...
int main() {
  ...
  float kkk;
  cout << sizeof(kkk);
  ifstream fff("C:\\1.bmp", ios::binary); // ios::binary влияет ТОЛЬКО на endl. 
  ...
  for (int i = 1; i <= h; ++i) { //вертикальный градиент, от step зависит, насколько высоко будет находиться переход
    for (int j = 1; j <= w; ++j) {
